...
export function VideoSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-navy-dark py-24 md:py-32" aria-labelledby="video-heading">
      <div className="absolute bottom-0 left-0 top-0 w-1 bg-glory-red" aria-hidden="true" />
      ...
      <div className="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <motion.p variants={fadeUp} className="max-w-sm font-body text-sm text-white/50">
        ...
        >
          {VIDEO_PREVIEWS.map((video, index) => (
            <motion.div
              ...
              role="button"
              tabIndex={0}
              onKeyDown={(event) => event.key === 'Enter' && setActiveVideo(video.id)}
              aria-label={`Play: ${video.title}`}
            >
              <div className={cn('relative w-full overflow-hidden', index === 0 ? 'aspect-video' : 'aspect-[4/3]')}>
                <Image
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  ...
                  >
                    <Play className="ml-1 h-6 w-6 fill-navy-dark text-navy-dark" aria-hidden="true" />
                  </motion.div>
                </div>
                ...
                <div className="absolute right-3 top-3 flex items-center gap-1 rounded-lg bg-black/60 px-2.5 py-1 backdrop-blur-sm">
                  ...
                  <span className="font-body text-xs font-medium text-white/80">{video.duration}</span>
                </div>
                ...
                <div className="absolute left-3 top-3 flex items-center gap-1 rounded-lg border border-glory-gold/30 bg-glory-gold/20 px-2.5 py-1 backdrop-blur-sm">
                  ...
                  <span className="font-body text-xs font-semibold text-glory-gold">{video.category}</span>
                </div>
              </div>

              <div className="bg-navy-light/80 p-4">
                <div className="mb-2 h-0.5 w-8 bg-glory-gold transition-all duration-300 group-hover:w-16" aria-hidden="true" />
            ...
            >
              <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
                <div>
                  ...
                  <h3 className="font-display text-xl text-white">{active.title}</h3>
                </div>
                <button
                  onClick={() => setActiveVideo(null)}
              ...
              <div className="relative flex flex-1 items-center justify-center bg-black">
                ...
                <div className="relative z-10 px-8 text-center">
                  <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full border-2 border-glory-gold bg-glory-gold/20">
                    <Play className="ml-1 h-8 w-8 fill-glory-gold text-glory-gold" aria-hidden="true" />
                  </div>
                  <p className="mb-2 font-display text-2xl text-white">{active.title}</p>
