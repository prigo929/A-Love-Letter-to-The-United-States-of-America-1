...
export function MapPreviewSection() {
  ...
  const handleMouseEnter = useCallback((geo: MapGeography, evt: React.MouseEvent<SVGPathElement>) => {
  ...
  return (
    <section className="relative overflow-hidden bg-navy-mid py-24 md:py-32" aria-labelledby="map-heading">
      <div className="bg-map-preview-grid absolute inset-0 pointer-events-none opacity-10" aria-hidden="true" />
      ...
      <div className="relative mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <motion.p variants={fadeUp} className="mx-auto max-w-xl font-body text-lg text-white/55">
            Hover any state to discover what makes it exceptional.
            Every state is a story.
          </motion.p>
        </motion.div>
        ...
        >
          <div className="pointer-events-none absolute inset-0 rounded-3xl bg-gradient-to-b from-glory-gold/5 to-transparent" aria-hidden="true" />
          ...
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-navy-dark/60 backdrop-blur-sm">
                    ...
                    return (
                      <Geography
                        ...
                        style={{
                          default: {
                            fill: isHovered
                              ? (hasFact ? COLORS.gloryGold : COLORS.gloryBlueLight)
                              : (hasFact ? COLORS.gloryBlue : COLORS.navyLight),
                          ...
                          hover: {
                            fill: hasFact ? COLORS.gloryGold : COLORS.gloryBlueLight,
            ...
            <div className="flex items-center justify-center gap-6 border-t border-white/10 px-6 py-4">
              <div className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-sm bg-glory-blue" aria-hidden="true" />
                <span className="font-body text-xs text-white/50">Featured state</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-sm bg-glory-gold" aria-hidden="true" />
                <span className="font-body text-xs text-white/50">Hover to explore</span>
              </div>
            </div>
          </div>
              ...
              >
                <div className="max-w-[260px] rounded-xl border border-glory-gold/30 bg-navy-dark px-4 py-3 shadow-2xl">
                  <div className="mb-1 flex items-center gap-2">
                    <MapPin className="h-3.5 w-3.5 shrink-0 text-glory-gold" aria-hidden="true" />
                    <span className="font-body text-sm font-semibold text-white">
                      {tooltip.name}
                      {tooltip.abbrev && (
                        <span className="ml-1.5 text-xs text-glory-gold">({tooltip.abbrev})</span>
                      )}
                    </span>
                  </div>
                  {tooltip.fact ? (
