...
export function FactCard({
  ...
  return (
    <motion.div
      ...
      className={cn('flex flex-col gap-3 rounded-2xl p-5', variantStyles[variant], className)}
    >
      <div className="flex items-center justify-between">
        <div className={cn('flex h-8 w-8 items-center justify-center rounded-lg border', colorAccents[color])}>
          {icon ?? <Zap className="h-4 w-4" aria-hidden="true" />}
        </div>
        {category && (
          <span className={cn('rounded-full border px-2.5 py-0.5 font-body text-xs font-semibold uppercase tracking-widest', colorAccents[color])}>
            {category}
          </span>
        )}
      </div>

      <p className="font-body text-base font-semibold leading-snug text-white">{fact}</p>

      {detail && (
        <p className="font-body text-sm leading-relaxed text-white/55">{detail}</p>
      )}
